  ...
  describe "#has_genre(word)" do
    context "when a valid word is given" do
      it "returns te same word" do
        expect(has_genre?("Drama")).to be(true)
      end
      it "returns included word" do
        expect(@genre.include?("Drama")).to include("Drama")
      end
    end

    context "when an incorrect word is given" do
      it "raises ArgumentError" do
        expect { has_genre?("Tragedy") }.to raise_error("Genre not found")
      end
    end
  end
end
