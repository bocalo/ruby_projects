...
class Product
	...
	def to_s
		"#{@quantity} #{@id}s that cost $#{@price} each" # для чего нужен метод to_s?
	end
end
...
# Сами методы понятны, а вот логика построения самого проекта нет. То есть мне нужно таких примеров побольше, чтобы понять как они строятся. 